...
/**
 * Find return statement value.
 * Using when linting component's data, and vuex's state.
 */
...
const filterUnsupportedProps = properties =>
  properties
    .filter(({ type }) => type !== "SpreadElement") // eslint 5+
    .filter(({ type }) => type !== "ExperimentalSpreadProperty") // eslint 4, babel-eslint
...
const isVueComputed = node =>
  isWithinVueComponent(node) && node.key.name === "computed"

const isVueMethods = node =>
  isWithinVueComponent(node) && node.key.name === "methods"
